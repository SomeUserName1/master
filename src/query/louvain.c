...
inline void removeNode(louvainPartition *p, adjlist *g, unsigned long node,
        unsigned long comm, long double dnodecomm) {
    p->in[comm] -= 2.0L * dnodecomm + selfloopWeighted(g, node);
    p->tot[comm] -= degreeWeighted(g, node);
}

inline void insertNode(louvainPartition *p, adjlist *g, unsigned long node,
        unsigned long comm, long double dnodecomm) {
    p->in[comm] += 2.0L * dnodecomm + selfloopWeighted(g, node);
    p->tot[comm] += degreeWeighted(g, node);
...
inline long double gain(louvainPartition *p, adjlist *g, unsigned long comm,
        long double dnc, long double degc) {
...
unsigned long updatePartition(louvainPartition *p, unsigned long *part,
        unsigned long size) {
...
adjlist *louvainPartition2Graph(louvainPartition *p, adjlist *g) {
    ...
    unsigned long *renumber =
        (unsigned long *)malloc(g->n * sizeof(unsigned long));
    ...
    res->e = 0;
    res->cd =
        (unsigned long long *)calloc((1 + res->n), sizeof(unsigned long long));
    res->cd[0] = 0;
    ...
    for (i = 0; i <= p->size; i++) {
        ...
        currentComm =
            (i == p->size) ? currentComm + 1 : p->node2Community[order[i]];
