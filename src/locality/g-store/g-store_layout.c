...
void insert_match(size_t* matches, size_t* match_weights, relationship_t* rel,
...
unsigned long compute_abs_tension(multi_level_graph_t* graph,
...
unsigned long compute_total_e_w_btw_blocks(multi_level_graph_t* graph,
...
long compute_num_e_btw_blocks(multi_level_graph_t* graph,
        const unsigned int* partition) {
...
long* compute_tension(multi_level_graph_t* graph, list_ul_t* nodes_in_p,
        bool modified_t) {
    ...
    for (size_t i = 0; i < num_nodes_p; ++i) {
        ...
        for (size_t j = 0; j < list_relationship_size(rels); ++j) {
            ...
            p2 = modified_t ?
                (long) graph->partition[finer->map_to_coarser[other_node_id]]
                : (long) finer->partition[other_node_id];

            tensions[i] += (long) (finer->edge_aggregation_weight[rel->id]
                    * (p2 - p1));
        }
        list_relationship_destroy(rels);
    }
...
unsigned long* sort_by_tension(list_ul_t* nodes, long *tensions,
        unsigned long size) {
    unsigned long i;
...
unsigned int* swap_partitions(multi_level_graph_t* graph, size_t idx1,
        size_t idx2) {
...
int coarsen(multi_level_graph_t* graph, size_t block_size,
        size_t* num_v_matches, size_t* max_partition_size,
        float* c_ratio_avg) {
    ...
    coarser->node_aggregation_weight = calloc(graph->records->node_id_counter,
            sizeof(unsigned long));

    coarser->edge_aggregation_weight = calloc(graph->records->rel_id_counter,
            sizeof(unsigned long));
    ...
    for (size_t i = 0; i < num_nodes; ++i) {
        ...
        for (size_t j = 0; j < num_rels; ++j) {
            rel = list_relationship_get(rels, j);
            other_node_id = i == rel->source_node ? rel->target_node
                : rel->source_node;
        ...
        num_matched = num_matched > *num_v_matches ? *num_v_matches
            : num_matched;

        for (size_t j = 0; j < num_matched; ++j) {
            node_matched[matches[j]] = true;
            coarser->node_aggregation_weight[i]
    ...
    for (unsigned long i = 0; i < num_rels; ++i) {
        ...
        if (graph->map_to_coarser[rel->source_node]
            ...
            new_rel = in_memory_contains_relationship_from_to(coarser->records,
                    ...
                    graph->map_to_coarser[rel->target_node], BOTH);


            if (new_rel == NULL) {
                new_rel_id = in_memory_create_relationship(coarser->records,
    ...
    c_ratio = (1.0F - (float) coarser->records->node_id_counter
    ...
    if (c_ratio < C_RATIO_LIMIT) {
        if (*num_v_matches == 2 && *max_partition_size
                <= MAX_PARTITION_SIZE_FACTOR * block_size) {
...
void project(multi_level_graph_t* graph, bool* part_type, size_t block_size,
    ...
    size_t num_nodes_p;
    long* tensions;
    float weight_threshold = block_size
    ...
    for (size_t coarser_part = 0; coarser_part < graph->num_partitions;
        ...
        unsigned long* sorted_nodes_p = sort_by_tension(nodes_coarser_p,
                tensions, num_nodes_p);
        ...
        for (size_t i = 0; i < num_nodes_p; ++i) {
            if (finer->partition_aggregation_weight[graph->num_partitions] > 0
                ...
                part_type[finer_partition] = i < num_nodes_p / 2 + 1 ? false
                    : true;
...
int uncoarsen(multi_level_graph_t* graph, size_t block_size, float c_ratio_avg) {
    ...
    for (size_t i = 0; i < graph->num_partitions; ++i) { nodes_per_part[i] = create_list_ul(); }
...
void g_store_layout(in_memory_file_t* db, size_t block_size) {
    ...
    graph->c_level = 0;
    graph->records = db;
    graph->node_aggregation_weight = malloc(db->node_id_counter * sizeof(unsigned long));
    graph->edge_aggregation_weight = malloc(db->rel_id_counter * sizeof(unsigned long));
    ...
    while (coarsen(graph, block_size, &num_v_matches, &max_partition_size,
                &c_ratio_avg) == 0) {
        graph = graph->coarser;
    }
