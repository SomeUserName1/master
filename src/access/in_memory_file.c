...
unsigned long in_memory_create_relationship(in_memory_file_t * db,
        unsigned long nodeFrom, unsigned long nodeTo) {
    return in_memory_create_relationship_weighted(db, nodeFrom, nodeTo, 1.0F);
}

unsigned long in_memory_create_relationship_weighted(in_memory_file_t * db,
        unsigned long nodeFrom, unsigned long nodeTo, double weight) {
    ...
    if (source_node->first_relationship == UNINITIALIZED_LONG) {
        ...
        while (next_id != UNINITIALIZED_LONG) {
            ...
            next_id = in_memory_next_relationship(db, source_node->id,
                    rel_source, BOTH);
        }
    }
    ...
    if (target_node->first_relationship == UNINITIALIZED_LONG) {
        ...
        while (next_id != UNINITIALIZED_LONG) {
            ...
            next_id = in_memory_next_relationship(db, target_node->id,
                    rel_target, BOTH);
        }
    }
...
unsigned long in_memory_next_relationship(in_memory_file_t* db,
        unsigned long node_id, relationship_t* rel, direction_t direction) {
    ...
    while (rel_id != UNINITIALIZED_LONG) {
        if (rel->source_node == node_id && (direction == BOTH
                    || direction == OUTGOING)) {
...
list_relationship_t* in_memory_expand(in_memory_file_t* db,
        unsigned long node_id, direction_t direction) {
    ...
    while (rel_id != UNINITIALIZED_LONG) {
        ...
        if ((rel->source_node == node->id && direction != INCOMING)
            ...
            printf("%s%lu%s%lu", "Expand: Invalid relationship ",
                    rel->id, " in chain of node ", node->id);
...
relationship_t* in_memory_contains_relationship_from_to(in_memory_file_t* db,
        unsigned long node_from, unsigned long node_to,
